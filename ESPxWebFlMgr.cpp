  ...
  if (ESPxWebFlMgr_FileSystem.exists(uri)) {
    ...
    if (f) {
      if (fileManager->streamFile(f, contentTyp) != f.size()) {
        // DEBUG(F("Sent less data than expected!"));
...
String ESPxWebFlMgr::dispFileString(size_t fs) {
  ...
  static const String units[] = { "B", "kB", "MB", "GB", "TB" };
...
String ESPxWebFlMgr::dircolorline(int i) {
  if (i % 2 == 0) {
    return "dcu";
  } else {
    return "dcg";
...
File ESPxWebFlMgr::firstFile(Dir &dir) {
  dir = ESPxWebFlMgr_FileSystem.openDir(dir.name());
  return nextFile(dir);
}
#endif
#ifdef ESP32
...
File ESPxWebFlMgr::firstFile(Dir &dir) {
  dir = ESPxWebFlMgr_FileSystem.open(dir.name(),"r");
  return nextFile(dir);
}
#endif
...
void ESPxWebFlMgr::fileManagerFileListInsert(void) {
  ...
  int i = 0;
  Dir dir;
  if ( (fileManager->args() == 1) && (fileManager->argName(0) == "dir") ) {
    dn = fileManager->arg(0) ;
    if (dn == "") {dn = "/";}
    uint8_t subdirlen = dn.lastIndexOf("/");
    if (subdirlen > 1) {
      pdn = dn.substring(0, subdirlen);
    } else {
      pdn = "/";
    }
    setcurdir(dn);
    dir = ESPxWebFlMgr_FileSystem.open(dn);
  }
...
//  DEBUG("fileManagerReceiver");

  HTTPUpload& upload = fileManager->upload();

//  DEBUG("Server upload Status: " + String(upload.status));

  if (upload.status == UPLOAD_FILE_START) {

    String filename = getcurdir() + "/" +  upload.filename;
    if (!filename.startsWith("/")) {
      filename = "/" + filename;
    }
//    DEB("handleFileUpload Name: ");
//    DEBUG(filename);
...
void ESPxWebFlMgr::fileManagerCommandExecutor(void) {
  ...
  /** /
    ...
    DEB(i);
    DEB(" ");
    DEB(fileManager->argName(i));
    DEB(": ");
    DEB(fileManager->arg(i));
    DEBUG();
    }
    /**/
  ...
  if ( (fileManager->args() == 1) && (fileManager->argName(0) == "del") ) {
    ...
    if ( (_ViewSysFiles) || (allowAccessToThisFile(fn)) ) {
      ...
      DEBUG("removing " + fn);
      if ( ESPxWebFlMgr_FileSystem.remove( fn) )   {    DEBUG("removed file " + fn); }
      else if ( ESPxWebFlMgr_FileSystem.rmdir( fn ) )  {     DEBUG("removed dir " + fn); }
      else { DEBUG("removing failed") ; }
    }
  }
